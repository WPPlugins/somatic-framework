...
class somaOptions extends somaticFramework  {
	...
	function init_soma_options() {
		...
		$defaults = array(
			...
			"go_redirect" => 0,
			"fulldisplayname" => 1,
			"extend_profile" => 1,						// additional user meta fields (online presence)
				...
				<?php settings_fields( 'somatic_framework_plugin_options' ); // adds hidden form elements, nonce ?>
					...
					<tr valign="top">
						...
						<td>
							...
							<label><input name="somatic_framework_options[extend_profile]" type="checkbox" value="1" <?php if ( isset( $soma_options['extend_profile'] ) ) { checked( '1', $soma_options['extend_profile'] ); } ?> /> Show additional contact meta fields in the User Profile</label><br />
					...
					<tr valign="top">
						...
						<td>
							<label><input name="somatic_framework_options[disable_metaboxes][]" type="checkbox" value="tb_page_options" <?php if ( is_array( $soma_options['disable_metaboxes'] ) ) { checked( '1', in_array( 'tb_page_options', $soma_options['disable_metaboxes'] ) ); } ?> /> ThemeBlvd Page Options</label><br />
...
?>
	...
	function extend_user_contactmethod( $contactmethods ) {
		global $soma_options;
		if ( somaFunctions::fetch_index( $soma_options, 'extend_profile' ) ) {
