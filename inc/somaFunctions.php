	...
	*/
	...
	function fetch_external_media($url = null, $width = null, $height = null) {
		...
		switch ($site) {
			case "youtube";
				if (preg_match('%(?:youtube\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $match)) {
				    ...
				    ob_start();
						...
						// $meta = file_get_contents("http://gdata.youtube.com/feeds/api/videos/".$id."?v=2&alt=json");  // old, don't work??
						$meta = file_get_contents("https://www.youtube.com/oembed?url=".$url."&format=json");
						$meta = json_decode($meta, true);	// json_decode true returns data as array instead of object
					ob_end_clean();
					
					$media['url']		= "http://www.youtube.com/watch?v=$id";																	// rebuild external url
					$media['id']		= $id;
					$media['site']		= $site;
					$media['thumb']		= $meta['thumbnail_url'];												// grabs url of medium 360x480 image
			...
			break;
			case "vimeo";
				// try to get around 403 forbidden errors when using file_get_contents
				ob_start();
					ini_set('user_agent','Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.56 Safari/537.17');
					$oembed = file_get_contents("https://vimeo.com/api/oembed.json?url=".urlencode($url));
					$oembed = json_decode($oembed, true);
				ob_end_clean();
				if (!empty($oembed)) {				// get oembed metadata
					$id = $oembed['video_id'];																								// extract id from api response
					ob_start();
						ini_set('user_agent','Mozilla/4.0 (compatible; MSIE 6.0)');
						$meta = file_get_contents("https://vimeo.com/api/v2/video/$id.php");
					ob_end_clean();
					$meta = array_shift(unserialize($meta));							// get more metadata (note: we're making two distinct calls to API for vimeo)
